...
Official releases provide binary packages built in multiple Linux environments
so users can choose the closest match for their runtime:

| Artifact target | When to use it |
| --- | --- | --- |
| `manylinux2014-x86_64` | glibc 2.17 baseline; prefer this for broad Linux compatibility. |
| `ubuntu20.04-x86_64` | Ubuntu 20.04 or OpenSSL 1.1 runtime environments. |
| `ubuntu22.04-x86_64` | Ubuntu 22.04 or OpenSSL 3 runtime environments. |
| `ubuntu24.04-x86_64` | Ubuntu 24.04 or newer runtime environments. |

Each package includes `out_aliyun_sls.so`, example config, README files, sha256
checksums, and `ldd`/`readelf` dependency metadata. Artifact files are named
`aliyun-log-fluent-bit-plugin-<target>-<arch>.tar.gz` and
`aliyun-log-fluent-bit-plugin-<target>-<arch>.sha256`, for example:
